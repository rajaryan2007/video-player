...
//double audio_start_pts = AV_NOPTS_VALUE;
//double video_start_pts = AV_NOPTS_VALUE;
//
//
//
//int main(void) {
...
//    avformat_open_input(&format_context, "C:\\Users\\rion\\Videos\\s3.mp4",
//        NULL, NULL
//    );
//
...
//
//    const int audio_bytes_per_sample = sizeof(float);
...
//                        double t = pts * av_q2d(format_context->streams[a_idx]->time_base);
//
...
//                    const uint64_t MAX_AUDIO_BUFFER =
//                        audio_bytes_per_second / 4;
//                    // 500 ms
//
//                    Uint64 queued = SDL_GetAudioStreamQueued(stream);
//                    if (queued < MAX_AUDIO_BUFFER) {
...
//                    }
//
//                    const double AUDIO_PREROLL = 0.2; // 200 ms
//                    double buffered_audio =
//                        (double)SDL_GetAudioStreamQueued(stream) / audio_bytes_per_second;
//
//                    if (buffered_audio < AUDIO_PREROLL) {
//                        goto render;
//                    }
//
//
...
//                        (double)played_bytes / audio_bytes_per_second;
//                    audio_clock = master_audio_clock;
//                    const double DEVICE_LATENCY = 0.05; // 50 ms guess
//
//                    double normalized_audio =
//                        master_audio_clock - audio_start_pts - DEVICE_LATENCY;
//
//                    double normalized_video =
//                        video_pts - video_start_pts;
//                    /* double video_time = video_pts - video_start_pts;
//                     double real_time = (SDL_GetTicks() - start_time) / 1000.0;
//
//                     double diff = video_time - real_time;*/
//                    double diff = normalized_video - normalized_audio;
//
//                    if (diff > 0.01) {
...
//
//                    if (diff > 0.01) {
//                        // Too early for this video frame? 
//                        // Don't Delay! Just break the video loop and read more packets
//                        // so we can fill the audio buffer.
//                        break;
//                    }
//                    else if (diff < -0.05) {
//                        continue; // Drop frame (too late)
//                    }
//
//                render:
//
...
//                    int w, h; SDL_GetCurrentRenderOutputSize(renderer, &w, &h); float scale = SDL_min((float)w / decoder->width, (float)h / decoder->height); SDL_FRect dstrect = { ((float)w - (decoder->width * scale)) / 2, ((float)h - (decoder->height * scale)) / 2, decoder->width * scale, decoder->height * scale };
